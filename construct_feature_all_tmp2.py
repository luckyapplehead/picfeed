...
train_set_s_term_raw= spark.read.csv("gs://dataproc-0e3e0110-db09-4037-98cc-dc355651aba0-asia-southeast1/tensorflow/data/picfeed/train_set_s_term/part-*", schema=fieldSchema)

print "finish read train_set_s_term"
#train_set_s_term_d = spark.createDataFrame(train_set_s_term_raw.rdd, ['label', 'pdef', 'pbeau', 'pnum', 's_term', 'sumclick', 'sumshow', 'ts', 'uid', 'urlid'])
train_set_s_term_raw.createOrReplaceTempView("train_set_s_term")
train_set_s_term_raw.head()
...
def combine_uinfo(a, b):
    ...
    term_dict_sorted = sorted(term_dict.iteritems(), key=lambda d: d[1], reverse=True)
    s=""
    term_count = 0
    for term_pair in term_dict_sorted:
        if len(s) > 0:
            s = s + ","
        s = s + term_pair[0] + ":" + str(term_pair[1])
