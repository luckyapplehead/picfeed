...
def process_train_set(line):
    ...
    if len(toparr) != 10:
        r = Row(uid=line.uid, urlid="-", label="-", ts="-", s_term="-", pnum=-1, pdef=-1, pbeau=-1)
        return r
    ...
    r = Row(uid=line.uid, urlid=line.urlid, label=line.label, ts=line.ts, s_term=s, pnum=pnum, pdef=pdef, pbeau=pbeau)
...
def construct_feature(line):
    ...
    r = Row(uid=line.uid, label=line.label, pnum=line.pnum, pdef=line.pdef, pbeau=line.pbeau, s_term_score=s_term_score)
    return r
